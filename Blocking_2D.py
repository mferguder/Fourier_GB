...
#----------------------------------------------------------------------
def Spectra_2D(qr,hq2t,nllq2t,nLq2t):
    z1=np.mean(hq2t,axis=0)*qr**4
    z2=np.mean(nllq2t,axis=0)*qr**2
    z3=np.mean(nLq2t,axis=0)
    dz1,bloxydys = Blocking(hq2t.T,qr)
    dz2,bloxydys = Blocking(nllq2t.T,qr)
    dz3,bloxydys = Blocking(nLq2t.T,qr)
    dz1*=qr**4
    dz2*=qr**2
